...
const Cart = () => {
    ...
    useEffect(()=>{
        dispatch(cartActions.setGetTotals())
    },[cartItems,dispatch])

    const onCartToggle = () =>{
        dispatch(cartActions.setCloseCart({
            cartState:false
        }))
    }
  ...
  return (
    <>
    <div 
    className={`fixed top-0 left-0 right-0 bottom-0 blur-effect-theme duration-500
        w-full h-screen opacity-100 z-[250] ${ifCartState ?
        'opacity-100 visible translate-x-0' :
        'opacity-0 invisible translate-x-8'}`}
        >
        <div className='blur-effect-theme h-screen max-w-xl w-full absolute right-0'>
            <CartCount onCartToggle={onCartToggle} quantityItems={quantityItems} onClearCartItem={onClearCartItem}/>
            {
